<x-container>
		<div {{ $attributes->merge(['class' => 'flex flex-col lg:flex-row items-center gap-8']) }}>
				...
				<div>
						@if ($profile)
								{!! wp_get_attachment_image($profile, 'medium', false, [
								    'class' => 'w-full h-full max-w-[330px] object-cover rounded-[60px]',
								]) !!}
						@endif
				</div>
						...
						@if ($title || $description)
								<div class="flex h-10 w-10 items-center justify-start">
										<svg width="800px" height="800px" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg"
												...
												<radialGradient id="IconifyId17ecdb2904d178eab21439" cx="-779.868" cy="686.689" r="91.008"
														gradientTransform="scale(1 -1) rotate(45 506.867 1318.897)" gradientUnits="userSpaceOnUse">
														<stop offset=".353" stop-color="#f9ddbd">
														</stop>
														<stop offset=".872" stop-color="#ffd29c">
														</stop>
												</radialGradient>
												<path
														...
														fill="url(#IconifyId17ecdb2904d178eab21439)">
								...
								<div class="flex items-center gap-4">
										@foreach ($socials as $social)
												@if ($social['link'] && $social['icon'])
														<a class="flex items-center gap-2 !no-underline" href="{{ $social['link']['url'] }}"
																@if ($social['link']['target']) target="{{ $social['link']['target'] }}" @endif
																class="flex h-10 w-10 items-center justify-center"
																@if ($social['link']['title']) title="{{ $social['link']['title'] }}" @endif>
																{!! wp_get_attachment_image($social['icon'], 'thumbnail', false, ['class' => 'w-6 h-6 object-contain']) !!}
																@if ($social['link']['title'])
																		{{ $social['link']['title'] }}
																@endif
														</a>
												@endif
